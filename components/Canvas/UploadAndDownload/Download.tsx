        ...
        function getMainSVG(formulaList: ChemicalOrganicFormula[]) {
            ...
            function getSVG(formulaList: ChemicalOrganicFormula[]) {
                ...
                formulaList.forEach(item => {
                    ...
                    const viewBox = item.getViewBoxSize();
                    const size = item.getSize();
                    
                    
                    const rotateSVG = `<svg class="CA_SVG" transform="rotate(${item.angle} ${x+size.width/2} ${y+size.height/2})" \
                        x="${x}" y="${y}" viewBox="0 0 ${viewBox.width} ${viewBox.height}" width="${size.width}" \
                        height="${size.height}" stroke="${item.color}">${item.getRawTemplate()}</svg>`;

                    switch(item.type) {
                        case "text":
                            mask += `<rect x="${x}" y="${y}" width="${size.width}" height="22" \
